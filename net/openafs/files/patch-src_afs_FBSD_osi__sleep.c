--- src/afs/FBSD/osi_sleep.c.orig	2016-12-08 04:01:51 UTC
...
@@ -104,6 +103,7 @@ afs_getevent(char *event)
 	afs_evhashcnt++;
...
+	cv_init(&newp->cond, "event cond var");
     ...
     newp->event = event;
@@ -125,7 +125,7 @@ afs_osi_Sleep(void *event)
     seq = evp->seq;
     while (seq == evp->seq) {
 	AFS_ASSERT_GLOCK();
...
@@ -159,13 +159,13 @@ afs_osi_TimedSleep(void *event, afs_int3
     ticks = tvtohz(&tv);
...
+	cv_timedwait(&evp->cond, &afs_global_sx, ticks);
     ...
     relevent(evp);
     return code;
@@ -180,7 +180,7 @@ afs_osi_Wakeup(void *event)
     evp = afs_getevent(event);
     if (evp->refcount > 1) {
 	evp->seq++;
-	wakeup(event);
